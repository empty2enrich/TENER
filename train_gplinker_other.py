...
from models.gplinker_other_im import GPLinker
from models.gplinker_other_sc import AutoModelGPLinker
...
def main():
    ...
    nums_file = 1
    # use_te = False
    model_continue = 'gplinker_weight_0.pth'
    
    
    # model
    # model = GPLinker(tags_mapping, bert_cfg_path, dim_embedding, dim_model, gp_head_size).to(device)
    model = AutoModelGPLinker.from_pretrained(bert_cfg_path, predicate2id=tags_mapping, head_size=gp_head_size, use_efficient=True).to(device)
    # 打印模型结构
    ...
    if is_train:
        ...
        for epoch in range(epochs):
            # todo : debug
            process_bar = tqdm(range(len(data_train)), desc='Trainning ')
            model.train()
            ...
            for step, batch in enumerate(data_train):
                ...
                if debug and steps_debug > -1 and step > steps_debug:
                    break
                input_idx, input_mask, label = [batch[k] for k in keys]
                # todo: to(device)
                input_idx = input_idx.to(device)
                input_mask = input_mask.to(device)
                label = [v.to(device) for v in label]
                
                # input_idx, segments, entity, head, tail = batch
                # input_idx = input_idx.to(device)
                # label_idx = label_idx.to(device)
                # segments = segments.to(device)
                out = model(input_idx, input_mask, label)
                loss = out[0]
                ...
                visual_tensorboard(log_dir_tsbd, f'train : ', {'loss': [loss.item()]}, epoch, step)
                loss.backward()
                scheduler.step()
                opt.step()
            ...
            # tp, tpfp, tpfn = 1, 1, 1
            # res_compare = []
            # for step, batch in tqdm(enumerate(data_test)):
            #     if debug and steps_debug > -1 and step > steps_debug:
            #         break
            ...
            #     # label_idx = label_idx.to(device)
            #     # segments = segments.to(device)
            #     e_pre, h_pre, t_pre = model.predict(input_idx, input_mask)
            ...
            #     #     tpfp += len(y_pre)
            #     #     tpfn += len(y_true)
            #     for y_pre, y_true in zip([e_pre, h_pre, t_pre], label):
